 ...
 */
/*
 * 1.Observable本身其实什么含义也没有，Observable没有什么可进行变换的，Observable本质上来说应该是
...
public class Observable<T> {
   ...
   /**
     ...
     */
    /*
     * rxjava的异步操作之所以可以做成是链式的，而不像普通异步回调是嵌套的，是因为使用了如下的方式：
     * 1.普通的异步回调形式：
     *  a.callback在注册到数据源的时候，只是一个注册的行为，不会触发数据源其他任何操作
     *  b.异步操作的启动是之后某个时刻调用数据源的某个方法进行的
     *  c.综合上面两点，普通的异步操作，注册回调和启动异步操作是两个不同时刻进行的动作，并且注册在前，
     *    启动在后，所以不容易实现代码的链式调用。
     *  d.普通的异步流程，只注册了一个回调，即处理结果的callback。
     *  * 在进行以上关于异步回调的分析过程中，发现其实分析的也不对，这种形式一样可以整合成链式的代码
     *    结构。通过builder设计模式。
     * 2.rxjava在整个流程中实际注册了两个回调，从而实现在subscribe()的时候同时进行两项操作：注册回调
     *   和触发异步操作。
     *   a.rxjava在创建Observable的时候，第一步是注册一个OnSubscribe对象到Observable中，最后在
     *     subscribe()的时候，又注册了一个处理结果的callback。通过这两个回调，在注册callback的时候，
     *     触发OnSubscribe对象的回调，进而同时触发异步流程。
     */
